...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", minHeight: "100vh", background: "#0c0e14" }}>
      ...
      `}</style>
      {role === "student"     && <StudentView     requests={requests} addRequest={addRequest} onLogout={() => setRole(null)} notifications={notifications} setNotifications={setNotifications} templates={templates} />}
      {role === "coordinator" && <CoordinatorView requests={requests} updateRequest={updateRequest} notifications={notifications} setNotifications={setNotifications} onLogout={() => setRole(null)} templates={templates} />}
      {role === "admin"       && <AdminView       requests={requests} updateRequest={updateRequest} notifications={notifications} setNotifications={setNotifications} onLogout={() => setRole(null)} templates={templates} setTemplates={setTemplates} />}
    </div>
  );
}
...
// ─── STUDENT VIEW ─────────────────────────────────────────────────────────────
function StudentView({ requests, addRequest, onLogout, notifications, setNotifications, templates }) {
...
// ─── ADMIN VIEW ───────────────────────────────────────────────────────────────
function AdminView({ requests, updateRequest, notifications, setNotifications, onLogout, templates, setTemplates }) {
  ...
  const [newTpl, setNewTpl] = useState({ name: "", category: "Pre-OJT", content: "", fields: [] });
      ...
      reader.onload = async (event) => {
        ...
        window.mammoth.extractRawText({ arrayBuffer })
          .then(result => {
            ...
            if (isEdit) setEditingTpl({ ...editingTpl, content: text, fields });
            else setNewTpl({ ...newTpl, content: text, fields });
          });
      };
  ...
  const saveTemplate = () => {
    const t = { ...newTpl, id: "t" + (templates.length + 1) };
    setTemplates(prev => [...prev, t]);
    setShowAddTpl(false);
    setNewTpl({ name: "", category: "Pre-OJT", content: "", fields: [] });
  };

  const updateExistingTemplate = () => {
    setTemplates(prev => prev.map(t => t.id === editingTpl.id ? editingTpl : t));
    setEditingTpl(null);
  };
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 20px" }}>
        ...
        {tab === "templates" && (
          <div className="slide-in">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h2 style={{ fontFamily: "'DM Serif Display',serif", fontSize: 20, color: "#dde2f0" }}>OJT Document Templates</h2>
              <button className="btn" onClick={() => setShowAddTpl(true)}
                style={{ padding: "8px 18px", background: "#e76f51", color: "#fff", borderRadius: 8, fontWeight: 700, fontSize: 13 }}>
                + Add New Template
              </button>
            </div>
            
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }} className="template-grid">
              {templates.map(t => (
                <div key={t.id} className="card" style={{ background: "#13151f" }}>

                  <div style={{ padding: "14px 20px", borderBottom: "1px solid #1e2130", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ fontWeight: 700, fontSize: 14, color: "#dde2f0" }}>{t.name}</span>
                      <span className="tag" style={{ background: `${CAT_COLORS[t.category]}15`, color: CAT_COLORS[t.category], fontSize: 10 }}>{t.category}</span>
                    </div>
                    <button className="btn" onClick={() => setEditingTpl(t)}
                      style={{ padding: "4px 10px", background: "#4f8ef715", border: "1px solid #4f8ef730", color: "#4f8ef7", borderRadius: 6, fontSize: 11, fontWeight: 600 }}>
                      View & Edit
                    </button>
                  </div>
                  <pre style={{ padding: "14px 20px", fontSize: 12, color: "#4b5472", whiteSpace: "pre-wrap", maxHeight: 100, overflow: "auto", fontFamily: "'DM Sans', sans-serif" }}>{t.content}</pre>
                  <div style={{ padding: "10px 20px", borderTop: "1px solid #1e2130", display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {t.fields.map(f => <span key={f} style={{ fontSize: 10, color: "#4f8ef7", background: "#4f8ef710", padding: "2px 6px", borderRadius: 4 }}>{f}</span>)}
                  </div>
                </div>
              ))}
            </div>
        ...
        {editingTpl && (
          <div className="fade-in" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.85)", zIndex: 600, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
            <div className="card slide-in" style={{ maxWidth: 550, width: "100%", padding: 24, background: "#13151f", border: "1px solid #252840" }}>
              <h2 style={{ fontSize: 18, color: "#4f8ef7", marginBottom: 20 }}>Edit OJT Template</h2>
              
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div>
                  <label style={{ fontSize: 11, fontWeight: 700, color: "#4b5472", textTransform: "uppercase", display: "block", marginBottom: 8 }}>Template Name</label>
                  <input className="inp" value={editingTpl.name} onChange={e => setEditingTpl({...editingTpl, name: e.target.value})} />
                </div>
                
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  <div>
                    <label style={{ fontSize: 11, fontWeight: 700, color: "#4b5472", textTransform: "uppercase", display: "block", marginBottom: 8 }}>Category</label>
                    <select className="inp" value={editingTpl.category} onChange={e => setEditingTpl({...editingTpl, category: e.target.value})}>
                      {Object.keys(CAT_COLORS).map(c => <option key={c}>{c}</option>)}
                    </select>
                  </div>
                  <div>
                    <label style={{ fontSize: 11, fontWeight: 700, color: "#4b5472", textTransform: "uppercase", display: "block", marginBottom: 8 }}>Update File (.docx)</label>
                    <input type="file" accept=".docx" onChange={(e) => handleFileUpload(e, true)} style={{ color: "#6b7590", fontSize: 12 }} />
                  </div>
                </div>

                <div>
                  <label style={{ fontSize: 11, fontWeight: 700, color: "#2a9d8f", textTransform: "uppercase", display: "block", marginBottom: 8 }}>Template Content</label>
                  <textarea className="inp" rows={8} value={editingTpl.content} onChange={e => setEditingTpl({...editingTpl, content: e.target.value})} style={{ fontSize: 12, lineHeight: 1.6 }} />
                  <div style={{ marginTop: 8, display: "flex", gap: 4, flexWrap: "wrap" }}>
                    {editingTpl.fields.map(f => <span key={f} className="tag" style={{ background: "#4f8ef715", color: "#4f8ef7" }}>{f}</span>)}
                  </div>
                </div>
              </div>

              <div style={{ display: "flex", gap: 10, marginTop: 24 }}>
                <button className="btn" onClick={() => setEditingTpl(null)} style={{ flex: 1, padding: "12px", background: "#1a1d2a", color: "#6b7590", borderRadius: 8 }}>Discard</button>
                <button className="btn" onClick={updateExistingTemplate}
                  style={{ flex: 2, padding: "12px", background: "#4f8ef7", color: "#fff", borderRadius: 8, fontWeight: 700 }}>Save Changes</button>
              </div>
            </div>
          </div>
        )}
        ...
        {showAddTpl && (
          <div className="fade-in" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.85)", zIndex: 600, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
            <div className="card slide-in" style={{ maxWidth: 550, width: "100%", padding: 24, background: "#13151f" }}>
              <h2 style={{ fontSize: 18, color: "#e76f51", marginBottom: 20 }}>Upload OJT Template</h2>
              
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div>
                  <label style={{ fontSize: 11, fontWeight: 700, color: "#4b5472", textTransform: "uppercase", display: "block", marginBottom: 8 }}>Template Name</label>
                  <input className="inp" placeholder="e.g. Liability Waiver" value={newTpl.name} onChange={e => setNewTpl({...newTpl, name: e.target.value})} />
                </div>
                
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  <div>
                    <label style={{ fontSize: 11, fontWeight: 700, color: "#4b5472", textTransform: "uppercase", display: "block", marginBottom: 8 }}>Category</label>
                    <select className="inp" value={newTpl.category} onChange={e => setNewTpl({...newTpl, category: e.target.value})}>
                      {Object.keys(CAT_COLORS).map(c => <option key={c}>{c}</option>)}
                    </select>
                  </div>
                  <div>
                    <label style={{ fontSize: 11, fontWeight: 700, color: "#4b5472", textTransform: "uppercase", display: "block", marginBottom: 8 }}>File (.docx)</label>
                    <input type="file" accept=".docx" onChange={handleFileUpload} style={{ color: "#6b7590", fontSize: 12 }} />
                  </div>
                </div>

                {newTpl.content && (
                  <div className="fade-in">
                    <label style={{ fontSize: 11, fontWeight: 700, color: "#2a9d8f", textTransform: "uppercase", display: "block", marginBottom: 8 }}>Detected Content & Placeholders</label>
                    <textarea className="inp" rows={6} value={newTpl.content} onChange={e => setNewTpl({...newTpl, content: e.target.value})} style={{ fontSize: 12, lineHeight: 1.6 }} />
                    <div style={{ marginTop: 8, display: "flex", gap: 4, flexWrap: "wrap" }}>
                      {newTpl.fields.map(f => <span key={f} className="tag" style={{ background: "#4f8ef715", color: "#4f8ef7" }}>{f}</span>)}
                    </div>
                  </div>
                )}
              </div>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 24 }}>
                <button className="btn" onClick={() => setShowAddTpl(false)} style={{ flex: 1, padding: "12px", background: "#1a1d2a", color: "#6b7590", borderRadius: 8 }}>Cancel</button>
                <button className="btn" onClick={saveTemplate} disabled={!newTpl.name || !newTpl.content}
                  style={{ flex: 2, padding: "12px", background: "#e76f51", color: "#fff", borderRadius: 8, fontWeight: 700 }}>Save to System</button>
              </div>
            </div>
          </div>
        )}
